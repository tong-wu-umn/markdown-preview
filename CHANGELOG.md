...
- **Plain-text rendering for `.txt` files.** Prose, hard-wrapped notes, `#` comments, and ASCII tables or art now display exactly as written, in a monospaced font, instead of being reflowed as Markdown. A `.txt` that looks like Markdown (frontmatter, a `# Heading`, a fenced code block, a `[link](url)`, or a table) still renders as Markdown. Switch any `.txt` with *View → Render as Markdown*; the choice is remembered per file. In plain text the outline stays empty, edit mode drops Markdown styling and the formatting bar, and export and Open in LLM use the plain text. Choose the document font instead with *Settings → General → Monospaced font for plain text*.
...
- **Collapsed Project Navigator folders stay collapsed when a file changes.** Editing the open file in another app reloaded it and re-expanded every folder above it, and an editor's save could expand every top-level folder. The navigator now keeps the folders you closed closed; opening a different file still reveals it in the tree.
